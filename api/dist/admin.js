...
router.get("/users", auth_1.authMiddleware, requireAdmin, async (req, res) => {
    try {
        const { page = 1, limit = 20, search, role, blocked, verified, sortBy = "createdAt", sortOrder = "desc" } = req.query;
        ...
        if (role === "user")
            where.AND = [{ isAdmin: false }, { isModerator: false }, { isAdvertiser: false }];
        if (blocked === "true")
            where.blocked = true;
        if (blocked === "false")
            where.blocked = false;
...
router.get("/users/:userId", auth_1.authMiddleware, requireAdmin, async (req, res) => {
    try {
...
router.delete("/users/:userId", auth_1.authMiddleware, requireAdmin, async (req, res) => {
    try {
        const { userId } = req.params;
...
router.post("/users/:userId/role", auth_1.authMiddleware, requireAdmin, async (req, res) => {
    try {
        const { userId } = req.params;
...
router.post("/users/:userId/block", auth_1.authMiddleware, requireAdmin, async (req, res) => {
    try {
        const { userId } = req.params;
        ...
        try {
            await (0, notify_1.sendTG)(targetUser.telegramId, `⛔ Ваш аккаунт заблокирован.\n\nПричина: ${reason}\n\nОбратитесь в поддержку @spectrmod для разблокировки.`);
        }
        catch (error) {
...
router.post("/users/:userId/unblock", auth_1.authMiddleware, requireAdmin, async (req, res) => {
    try {
        const { userId } = req.params;
        const updatedUser = await prisma_1.prisma.user.update({
            where: { id: userId },
        ...
        try {
            await (0, notify_1.sendTG)(updatedUser.telegramId, `✅ Ваш аккаунт разблокирован!\n\nВы снова можете пользоваться приложением.`);
        }
        catch (error) {
...
router.get("/news", auth_1.authMiddleware, requireAdmin, async (req, res) => {
    try {
        const [news, total] = await prisma_1.prisma.$transaction([
            prisma_1.prisma.news.findMany({ orderBy: { createdAt: 'desc' } }),
...
// Создать новость
router.post("/news", auth_1.authMiddleware, requireAdmin, async (req, res) => {
    try {
...
// Обновить новость
router.put("/news/:id", auth_1.authMiddleware, requireAdmin, async (req, res) => {
    try {
        const { id } = req.params;
        const { title, content } = req.body;
        if (!title || !content) {
        ...
        const news = await prisma_1.prisma.news.update({ where: { id }, data: { title, content } });
        res.json(news);
    }
    catch (error) {
...
// Удалить новость
router.delete("/news/:id", auth_1.authMiddleware, requireAdmin, async (req, res) => {
    try {
        const { id } = req.params;
...
router.post("/news/:id/publish", auth_1.authMiddleware, requireAdmin, async (req, res) => {
    try {
        const { id } = req.params;
        // Получаем новость
        ...
        const usersToNotify = users.filter(user => {
            if (!user.settings)
                return true; // Если нет настроек - отправляем
            if (news.type === 'news' && user.settings.notifyNews)
                return true;
            if (news.type === 'tech_update' && user.settings.notifyTechUpdates)
                return true;
            return false;
        });
        ...
        const emoji = priorityEmoji[news.priority] || '📢';
        ...
        for (let i = 0; i < usersToNotify.length; i += batchSize) {
            ...
            await Promise.all(batch.map(async (user) => {
                try {
                    await (0, notify_1.sendTG)(user.telegramId, message);
                    successCount++;
                }
                catch (error) {
...
router.post("/users/:userId/message", auth_1.authMiddleware, requireAdmin, async (req, res) => {
    try {
        const { userId } = req.params;
        ...
        try {
            await (0, notify_1.sendTG)(targetUser.telegramId, `${senderName}\n\n${message}`);
        }
        catch (error) {
...
router.get("/stats", auth_1.authMiddleware, requireAdmin, async (req, res) => {
    try {
        const [totalUsers, totalProfiles, blockedUsers, verifiedProfiles, totalMatches, totalMessages, pendingComplaints, activeNews] = await Promise.all([
            ...
            prisma_1.prisma.news.count({ where: { isPublished: true } })
        ]);
        const [adminCount, moderatorCount, advertiserCount] = await Promise.all([
            prisma_1.prisma.user.count({ where: { isAdmin: true } }),
            prisma_1.prisma.user.count({ where: { isModerator: true } }),
            prisma_1.prisma.user.count({ where: { isAdvertiser: true } })
        ]);
        res.json({
            users: {
                total: totalUsers,
...
// ===== АНАЛИТИКА =====
router.get("/analytics", auth_1.authMiddleware, requireAdmin, async (req, res) => {
    try {
        ...
        const [totalUsers, newUsers, activeChats, totalLikes, totalMatches, totalMessages, reportsPending, reportsTotal, profilesModeration,] = await Promise.all([
            prisma_1.prisma.user.count(),
            prisma_1.prisma.user.count({ where: { createdAt: { gte: since } } }),
            prisma_1.prisma.chat.findMany({
                where: { updatedAt: { gte: new Date(new Date().setHours(new Date().getHours() - 24)) } },
                select: { userAId: true, userBId: true },
            }),
            prisma_1.prisma.like.count({ where: { createdAt: { gte: since } } }),
            prisma_1.prisma.match.count({ where: { createdAt: { gte: since } } }),
            prisma_1.prisma.message.count({ where: { createdAt: { gte: since } } }),
            prisma_1.prisma.complaint.count({ where: { status: 'pending' } }),
            prisma_1.prisma.complaint.count({ where: { createdAt: { gte: since } } }),
            prisma_1.prisma.profile.count({ where: { isFlagged: true, moderatedAt: null } }),
        ]);
...
router.get("/complaints", auth_1.authMiddleware, requireAdmin, async (req, res) => {
    try {
...
router.post("/complaints/:id/action", auth_1.authMiddleware, requireAdmin, async (req, res) => {
    try {
        const { id } = req.params;
