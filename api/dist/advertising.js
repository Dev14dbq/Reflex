...
const requireAdvertiser = async (req, res, next) => {
    try {
...
router.get("/stats", auth_1.authMiddleware, requireAdvertiser, async (req, res) => {
    try {
...
router.get("/campaigns", auth_1.authMiddleware, requireAdvertiser, async (req, res) => {
    try {
        const { page = 1, limit = 20, status, sortBy = "createdAt", sortOrder = "desc" } = req.query;
        const skip = (Number(page) - 1) * Number(limit);
        const where = { advertiserId: req.userId };
        if (status)
            where.status = status;
        const orderBy = {};
...
router.get("/campaigns/:campaignId", auth_1.authMiddleware, requireAdvertiser, async (req, res) => {
    try {
        const { campaignId } = req.params;
        const campaign = await prisma_1.prisma.adCampaign.findFirst({
            where: {
                id: campaignId,
...
router.post("/campaigns", auth_1.authMiddleware, requireAdvertiser, async (req, res) => {
    try {
        // Проверяем, есть ли уже кампания у пользователя
        const existingCampaign = await prisma_1.prisma.adCampaign.findFirst({
            where: { advertiserId: req.userId }
        });
        if (existingCampaign) {
            res.status(400).json({
        ...
        const { title, adTitle, adDescription, adImageUrl, adButtonText, adButtonUrl, targetAgeMin, targetAgeMax, targetGender, targetArea, startDate, endDate } = req.body;
...
router.post("/campaigns/:campaignId/status", auth_1.authMiddleware, requireAdvertiser, async (req, res) => {
    try {
        const { campaignId } = req.params;
        ...
        const updatedCampaign = await prisma_1.prisma.adCampaign.update({
            where: { id: campaignId },
...
router.get("/campaigns/:campaignId/analytics", auth_1.authMiddleware, requireAdvertiser, async (req, res) => {
    try {
        const { campaignId } = req.params;
        ...
        const analytics = await prisma_1.prisma.adAnalytic.findMany({
            where: {
                campaignId,
...
router.get("/analytics/overview", auth_1.authMiddleware, requireAdvertiser, async (req, res) => {
    try {
        const campaigns = await prisma_1.prisma.adCampaign.findMany({
            where: { advertiserId: req.userId }
        });
...
router.get("/targeting/audience", auth_1.authMiddleware, requireAdvertiser, async (req, res) => {
    try {
        ...
        const audienceSize = await prisma_1.prisma.profile.count({
            where: {
                ...where,
                user: { blocked: false }
            }
...
router.get("/serve", async (req, res) => {
    try {
        ...
        let clickedCampaignIds = [];
        if (userId) {
            const userClicks = await prisma_1.prisma.adUserClick.findMany({
        ...
        const suitableCampaigns = campaigns.filter(campaign => {
            if (age && campaign.targetAgeMin && Number(age) < campaign.targetAgeMin)
                return false;
            if (age && campaign.targetAgeMax && Number(age) > campaign.targetAgeMax)
                return false;
            if (gender && campaign.targetGenders.length > 0 && !campaign.targetGenders.includes(gender))
                return false;
            if (city && campaign.targetCities.length > 0) {
                const cityMatch = campaign.targetCities.some(targetCity => city.toLowerCase().includes(targetCity.toLowerCase()));
                if (!cityMatch)
                    return false;
            }
            return true;
...
router.get("/campaign/:campaignId/ad", async (req, res) => {
    try {
        const { campaignId } = req.params;
        const campaign = await prisma_1.prisma.adCampaign.findUnique({
            where: { id: campaignId }
        });
...
router.post("/track/impression", async (req, res) => {
    try {
        ...
        await prisma_1.prisma.adAnalytic.upsert({
            where: {
                campaignId_date: {
                    campaignId,
        ...
        await prisma_1.prisma.adCampaign.update({
            where: { id: campaignId },
...
router.post("/track/click", async (req, res) => {
    try {
        ...
        if (userId) {
            ...
            if (userExists) {
                await prisma_1.prisma.adUserClick.upsert({
                    where: {
                        userId_campaignId: {
...
router.get("/admin/campaigns", auth_1.authMiddleware, requireAdmin, async (req, res) => {
    try {
        const campaigns = await prisma_1.prisma.adCampaign.findMany({
            include: {
                advertiser: {
...
// Модерация кампании
router.post("/admin/campaigns/:campaignId/moderate", auth_1.authMiddleware, requireAdmin, async (req, res) => {
    try {
        const { campaignId } = req.params;
...
exports.advertisingRouter = router;
