...
authRouter.post("/auth/by-initdata", async (req, res) => {
    try {
        const { initData } = req.body;
        const { ok, hash, user } = (0, validate_1.validateInitData)(initData, process.env.BOT_TOKEN);
        if (!ok || !hash || !user) {
        ...
        let dbUser = await prisma_1.prisma.user.findUnique({ where: { hash } });
        // 2. Если не нашёлся — ищем по telegramId
        if (!dbUser) {
            const byTelegramId = await prisma_1.prisma.user.findFirst({ where: { telegramId } });
            if (byTelegramId) {
                // Обновляем hash (на новое устройство)
                try {
                    dbUser = await prisma_1.prisma.user.update({
        ...
        if (!dbUser) {
            try {
                ...
                if (error.code === 'P2002' && error.meta?.target?.includes('hash')) {
                    ...
                    dbUser = await prisma_1.prisma.user.findUnique({ where: { hash } });
                    if (!dbUser) {
        ...
        const token = jsonwebtoken_1.default.sign({ userId: dbUser.id }, JWT_SECRET, { expiresIn: "30d" });
        res.json({
            user: {
                ...dbUser,
