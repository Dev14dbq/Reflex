...
exports.moderationRouter = void 0;
...
async function getModerationSystemUser() {
    let moderationUser = await prisma_1.prisma.user.findFirst({
        where: {
    ...
    if (!moderationUser) {
        ...
        moderationUser = await prisma_1.prisma.user.findUnique({
            where: { id: newUser.id },
        ...
        moderationUser.profile = await prisma_1.prisma.profile.findUnique({
...
router.get("/complaints", auth_1.authMiddleware, requireModerator, async (req, res) => {
    try {
        const { page = 1, limit = 20, status, priority, type, assignedTo, sortBy = "createdAt", sortOrder = "desc" } = req.query;
...
router.post("/complaints/:complaintId/assign", auth_1.authMiddleware, requireModerator, async (req, res) => {
    try {
        const { complaintId } = req.params;
        const { moderatorId } = req.body;
        // Проверяем, что модератор существует и имеет права
        const moderator = await prisma_1.prisma.user.findUnique({
...
// Разрешить жалобу
router.post("/complaints/:complaintId/resolve", auth_1.authMiddleware, requireModerator, async (req, res) => {
    try {
        const { complaintId } = req.params;
        ...
        await prisma_1.prisma.moderatorAction.create({
            data: {
...
router.get("/profiles", auth_1.authMiddleware, requireModerator, async (req, res) => {
    try {
        const { page = 1, limit = 20, status = "all", sortBy = "user", sortOrder = "desc" } = req.query;
        const skip = (Number(page) - 1) * Number(limit);
        // Построение условий фильтрации
        const where = {};
        ...
        if (status === "pending")
            where.AND = [{ isVerified: false }, { isFlagged: false }];
...
router.post("/profiles/:profileId/verify", auth_1.authMiddleware, requireModerator, async (req, res) => {
    try {
...
router.post("/users/:userId/message", auth_1.authMiddleware, requireModerator, async (req, res) => {
    try {
        ...
        const systemMessage = await prisma_1.prisma.message.create({
            data: {
                chatId: chat.id,
        ...
        try {
            await (0, notify_1.sendTG)(targetUser.telegramId, `⚖️ Модерация\n\n${message}`);
        }
        catch (error) {
...
router.get("/stats", auth_1.authMiddleware, requireModerator, async (req, res) => {
    try {
        const today = new Date();
        today.setHours(0, 0, 0, 0);
        const [totalComplaints, pendingComplaints, resolvedComplaints, totalProfiles, verifiedProfiles, flaggedProfiles, totalImages, todayActions, profilesForReview, flaggedImages] = await Promise.all([
            prisma_1.prisma.complaint.count(),
            prisma_1.prisma.complaint.count({ where: { status: "pending" } }),
            prisma_1.prisma.complaint.count({ where: { status: "resolved" } }),
            prisma_1.prisma.profile.count(),
            prisma_1.prisma.profile.count({ where: { isVerified: true } }),
            prisma_1.prisma.profile.count({ where: { isFlagged: true } }),
            prisma_1.prisma.image.count(),
            prisma_1.prisma.moderatorAction.count({ where: { createdAt: { gte: today } } }),
            prisma_1.prisma.profile.count({ where: { isVerified: false, isFlagged: false } }),
            prisma_1.prisma.image.count({ where: { isApproved: false } })
        ]);
        res.json({
            pendingComplaints,
            profilesForReview,
...
async function performModerationAction(moderatorId, userId, action, reason) {
    switch (action) {
        case "warn":
            const user = await prisma_1.prisma.user.findUnique({
                where: { id: userId },
            ...
            if (user) {
                await (0, notify_1.sendTG)(user.telegramId, `⚠️ Предупреждение от модерации\n\n${reason}\n\nСледующее нарушение может привести к блокировке аккаунта.`);
            }
            break;
        case "block":
...
router.get("/profiles/next", auth_1.authMiddleware, requireModerator, async (req, res) => {
    try {
...
router.get("/users/search", auth_1.authMiddleware, requireModerator, async (req, res) => {
    try {
        const { q } = req.query;
...
router.get("/users/:userId", auth_1.authMiddleware, requireModerator, async (req, res) => {
    try {
        const { userId } = req.params;
        const user = await prisma_1.prisma.user.findUnique({
            where: { id: userId },
        ...
        const [complaintsAgainst, complaintsBy, moderatorActions] = await Promise.all([
            prisma_1.prisma.complaint.count({ where: { userId } }),
            prisma_1.prisma.complaint.count({ where: { reporterId: userId } }),
            prisma_1.prisma.moderatorAction.count({ where: { userId } })
        ]);
        // Исключаем telegramId для избежания ошибки сериализации BigInt
        const { telegramId, ...userWithoutTelegramId } = user;
        res.json({
...
router.post("/users/:userId/ban", auth_1.authMiddleware, requireModerator, async (req, res) => {
    try {
        const { userId } = req.params;
        ...
        // Уведомляем пользователя
        try {
            await (0, notify_1.sendTG)(user.telegramId, `🚫 Ваш аккаунт заблокирован\n\nПричина: ${reason}\n\nЕсли вы считаете это ошибкой, обратитесь в поддержку.`);
        }
        catch (error) {
...
router.post("/users/:userId/unban", auth_1.authMiddleware, requireModerator, async (req, res) => {
    try {
        const { userId } = req.params;
        const user = await prisma_1.prisma.user.findUnique({
            where: { id: userId },
        ...
        // Уведомляем пользователя
        try {
            await (0, notify_1.sendTG)(user.telegramId, `✅ Ваш аккаунт разблокирован\n\nВы снова можете пользоваться приложением.`);
        }
        catch (error) {
...
router.post("/chats/start", auth_1.authMiddleware, requireModerator, async (req, res) => {
    try {
...
router.get("/chats", auth_1.authMiddleware, requireModerator, async (req, res) => {
    try {
        // Получаем системный аккаунт модерации
        const moderationUser = await getModerationSystemUser();
...
router.get("/chats/:userId/messages", auth_1.authMiddleware, requireModerator, async (req, res) => {
    try {
        const { userId } = req.params;
...
router.post("/chats/:userId/send", auth_1.authMiddleware, requireModerator, async (req, res) => {
    try {
        const { userId } = req.params;
        const { message } = req.body;
        ...
        try {
            await (0, notify_1.sendTG)(targetUser.telegramId, `⚖️ Сообщение от модерации:\n\n${message.trim()}`);
        }
        catch (error) {
...
router.post("/complaints/:complaintId/action", auth_1.authMiddleware, requireModerator, async (req, res) => {
    try {
        const { complaintId } = req.params;
...
exports.moderationRouter = router;
