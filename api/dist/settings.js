...
settingsRouter.get("/settings", auth_1.authMiddleware, async (req, res) => {
...
settingsRouter.put("/settings", auth_1.authMiddleware, async (req, res) => {
    const userId = req.userId;
    const { notifyMessages, notifyLikes, notifyNews, notifyAds, notifyTech, sameCityOnly, } = req.body;
    try {
        ...
        if (sameCityOnly !== undefined)
            updateData.sameCityOnly = sameCityOnly;
...
settingsRouter.patch("/settings/notifications", auth_1.authMiddleware, async (req, res) => {
    const userId = req.userId;
    const { notifyMessages, notifyLikes, notifyNews, notifyAds, notifyTech, } = req.body;
    try {
...
settingsRouter.patch("/settings/recommendations", auth_1.authMiddleware, async (req, res) => {
    const userId = req.userId;
...
settingsRouter.get("/settings/recommendations", auth_1.authMiddleware, async (req, res) => {
    const userId = req.userId;
    try {
        let settings = await prisma_1.prisma.settings.findUnique({
            where: { userId },
...
settingsRouter.put("/settings/recommendations", auth_1.authMiddleware, async (req, res) => {
    const userId = req.userId;
    const { similarAge, localFirst, showNsfw, sameCityOnly, ageRangeMin, ageRangeMax, maxDistance } = req.body;
    try {
        const updated = await prisma_1.prisma.settings.upsert({
            where: { userId },
            update: {
                similarAge,
                localFirst,
