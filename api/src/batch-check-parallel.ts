...
import { checkImageNsfw, initializeNsfwModel } from "./nsfw";
import axios from "axios";

// Количество параллельных проверок
const PARALLEL_LIMIT = 5;

async function processImageBatch(images: { url: string; profileId: string; order: number }[]): Promise<{
  checked: number;
  nsfw: number;
  errors: number;
}> {
...
async function batchCheckParallel() {
  ...
  try {
    ...
    const imagesToCheck: { url: string; profileId: string; order: number }[] = [];
