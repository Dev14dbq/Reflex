...
locationRouter.post("/location/suggest-cities", authMiddleware, async (req, res): Promise<void> => {
  try {
...
locationRouter.get("/location/reverse-geocode", authMiddleware, async (req, res): Promise<void> => {
  try {
...
locationRouter.post("/location/search-places", authMiddleware, async (req, res): Promise<void> => {
  try {
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        response = await fetch(
          `https://api.geoapify.com/v1/geocode/search?${searchParams}`,
          {
            headers: {
              'User-Agent': 'Reflex-Dating-App/1.0'
            },
...
locationRouter.get("/location/popular-cities", authMiddleware, async (req, res): Promise<void> => {
  try {
