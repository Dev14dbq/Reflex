...
export async function startWebSocketServer(ws: WebSocket, req: IncomingMessage) {
  let userId: string;
  try {
  ...
  const getRecommendations = async () => {
    ...
    const likedProfileIds = await prisma.like.findMany({
    ...
    try {
      ...
      profiles.sort((a, b) => {
        ...
        if (settings?.localFirst) {
          ...
          if (a.city === userProfile.city) scoreA *= 1.2;
          if (b.city === userProfile.city) scoreB *= 1.2;
        }
      ...
      profiles.sort((a, b) => {
        ...
        if (settings?.localFirst) {
          if (a.city === userProfile.city) scoreA += 5;
          if (b.city === userProfile.city) scoreB += 5;
        }

        // Приоритет по полноте профиля
        if (a.description.length > 50) scoreA += 2;
        if (b.description.length > 50) scoreB += 2;
        if (a.images.length >= 3) scoreA += 2;
        if (b.images.length >= 3) scoreB += 2;
